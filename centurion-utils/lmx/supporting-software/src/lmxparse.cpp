...
//
// Version: 5.6
//      - Writer_format_compatibility: Formatting of attributes is now separate
//        to that of elements.  To allow for backwards compatibility, if the
//        attribute and element formatting characters are the same, then the
//        attribute formatting characters are changed to match the element
//        formatting characters.
//
// Version: 5.5
//      - !C110316_1000: Bad end tag error in
//        c_unmarshal_helper::unmarshal_simple_content_case() now reported to
//        error handler.
//      - !C110316_1024: Detected errors in lexical_validator_empty() now
//        reported to error handler.
//
// Version: 5.4
//      - !C101112_2112: Refactored order of code in
//        c_get_with_entities::select_general_entity().
//
// Version: 5.3
//      - !C100928_1704: Additional check for empty string made when reading
//        internal DTD.
//      - !C101103_0904: Fixed vc10 out of member exception handling problem.
//
// Version: 5
//      - Reformatted.
//      - Added c_unmarshal_helper class.
//      - !C100307_1217: xml-decl standalone attribute is now only treated as
//        a hint and is now ignored.
//      - !C100316_1735: Added c_xml_writer::position_attribute() calls to
//        c_xml_writer::conditionally_write_ns_attrs()
//
// Version: 4.2
//      - Added check of c_error instance in c_xml_reader::handle_error() when
//        ELMX_UNEXPECTED_EOM is reported and a fatal error has been recorded.
//      - Removed code commented out as part of ns_none_fix_del_check.
//      - Added additional ()s to (a && b || c && d) type expressions to suppress
//        over zealous GCC 4.3.3+ warnings.  See gcc_433_brackets.
//
// Version: 4.1
//      - ns_none_fix - c_xml_local::find_namespace_id() modified so that when
//        an XML namespace has no prefix and a search for a corresponding
//        namespace yields EXKN_UNKNOWN, then EXKN_NONE is returned.
//        This fix has enabled some code of the form if( NS_Unknown and prefix
//        empty ) then use NS_None to be removed.  See ns_none_fix_del_check
//        for where this has been done.
//      - Modified c_xml::get_named_attribute() so that it can look for
//        attributes that do not specify a namespace prefix. See Named_attr_fix.
//      - Added c_cracked_name::set_for_attribute(...).
//
// Version: 3.7.5
//      - Added methods related to the autover2 feature, including
//        c_known_elements methods, and c_xml_reader::get_known_element_event().
//
// Version: 3.6.1
...
//      - Corrected handling of the xmlns="" case.
//
// Version: 3.5
...
//
// Version: 3.2.4
//      - Added LMX_XML_STRICT compiler option to select strict XML entity
//        handling and checking of illegal UTF-8 character sequences.
//
// Version: 3.2
//      - Added ELMX_VALUE_DOES_NOT_MATCH_FIXED_CONSTRAINT error explanation.
//
// Version: 3.1
...
//
// Version: 2.8
//      - Unicode functions now use signed char rather than unsigned char.
//      - Added bswap (byte swap) utility function.
//
// Version: 2.7
//      - History log in lmxparse.h
//
...
//
// Notes:   get_semantics - There are a number of classes that can be used to
//          get character input in one form or another.  For convenience they
//          all have the following properties.  They all return an int value
//          even though the actual data is limited to unsigned 8-bit values.
//          Care must be taken to avoid the sign of char values being extended
//          to the higher bits of the int value.  c_read::k_eof is used to
//          indicate end of input.  At the end of input c_read::k_eof can be
//          read multiple times if required.  (This mainly affects the design
//          decision for the c_attribute_data store.)  It is possible to
//          safely unget() c_read::k_eof (multiple times if required).
//
//          xml_1_1_namespace_disabling - XML 1.1 allows a namespace prefix to
//          be disabled by doing xmlns:tns=''.  We do this by mapping the
//          namespace prefix to EXKN_NONE.
//------------------------------------------------------------------------------
