...
namespace engine {
	...
	void Camera::mouseControl(float threshold) {
		...
		threshold_x = mapWidth - 2 * Settings::CameraMovespeed() + (myWindow::WidthZoomed - myWindow::Width);
		threshold_y = mapHeight - 2 * Settings::CameraMovespeed() + (myWindow::HeightZoomed - myWindow::Height);
		...
		if (!gameMenuStatus && (KeyCode[GLFW_KEY_LEFT_CONTROL] || KeyCode[GLFW_KEY_RIGHT_CONTROL]) && Mouse::ScrollBool) {
			...
			else if (Mouse::ScrollValue < 0 && currentZoomCamera < Settings::CameraMaxZoom()) {
				currentZoomCamera -= (int)Mouse::ScrollValue;
			}
			Mouse::ScrollBool = false;
		}

		if (position.x > mapWidth - myWindow::WidthZoomed - Settings::CameraMovespeed()) {
			position.x = mapWidth - myWindow::WidthZoomed - Settings::CameraMovespeed();
		}
