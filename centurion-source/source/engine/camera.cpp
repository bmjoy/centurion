...
void Camera::keyboardControl() {
	//Left margin 
	if (getBoolean("left-key") && position.x > 0) {
		position -= right * cameraMovespeed;
	}
	//Right margin 
	if (getBoolean("right-key") && position.x < mapWidth - getParam("window-width-zoomed")) {
		position += right * cameraMovespeed;
	}
	//Bottom margin 
	if (getBoolean("up-key") && (position.y < (mapHeight - getParam("window-height-zoomed") + getParam("ui-top-height")* getParam("window-height-zoomed") / getParam("window-height")))) {
		position += up * cameraMovespeed;
	}
	//Bottom margin 
	if (getBoolean("down-key") && (position.y > (0 - getParam("ui-bottom-height")* getParam("window-height-zoomed") / getParam("window-height")))) {
		position -= up * cameraMovespeed;
	}
