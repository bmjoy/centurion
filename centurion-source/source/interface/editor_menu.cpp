    ...
    void EditorMenu::create() {
        ...
        for (int i = 0; i < titles.size(); i++) {
            ...
            for (int j = 0; j < titles[s].options.size(); j++) {
                ...
                tempText.create_static(titles[s].options[j], "tahomabold_13px", titles[s].optionsPosition[j].x + 0.5f * titles[s].optionsWidth - titles[s].optionsOffsetX, titles[s].optionsPosition[j].y + 0.5f * titles[s].optionsHeight, "left", "middle", glm::vec4(255.f));
