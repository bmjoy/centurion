...
#define __xmlRaiseError(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q) {(void)k, (void)l; (void)n;}
#define xmlMalloc(s) malloc(s)
#define xmlFree(p) free(p)
#define xmlRealloc(p,s) realloc(p,s)
#define xmlMallocAtomic(s) malloc(s)
#define xmlStrEqual(x,y) (strcmp((char*)x,(char*)y)==0)
#define xmlGenericError(a, b, c, d)
#define xmlErrEncodingInt(a, b, c, d);
#define xmlErrMemory(a, b)
...
static xmlChar *
xmlStrdup(const xmlChar *s)     // v3.9 - added
{
    // strdup() is non-standard and we want to be sure we use malloc() instead of new()
    xmlChar *copy = (xmlChar *)xmlMalloc( strlen( (const char *)s ) + 1 );
    if( copy )
        strcpy( (char *)copy, (const char *)s );
    return copy;
}
 ...
 */
xmlChar *
xmlStrndup(const xmlChar *cur, LMXSTDCRT::ptrdiff_t len) {
    xmlChar *ret;
...
int
xmlStringCurrentCharUTF8(const xmlChar * cur, int *len)
{
    unsigned char c;
    unsigned int val;
...
#define xmlStringCurrentChar(c,s,l) xmlStringCurrentCharUTF8(s,l)
...
c_regex::c_regex( const std::string & r_pattern )
{
    mp_regex = xmlRegexpCompile( reinterpret_cast<const xmlChar*>( r_pattern.c_str() ) );
}

bool c_regex::is_match( const std::string & r_s ) const
{
    return mp_regex && xmlRegexpExec( mp_regex, reinterpret_cast<const xmlChar*>( r_s.c_str() ) );
}

c_regex::~c_regex()
{
    if( mp_regex )
        xmlRegFreeRegexp( mp_regex );
}
 ...
 */
static xmlRegexpPtr
xmlRegExpFromParse(xmlRegParserCtxtPtr ctxt) {
    xmlRegexpPtr ret;
...
#endif
        stringMap = (xmlChar **)xmlMalloc(ret->nbAtoms * sizeof(char *));
        if (stringMap == NULL) {
            xmlRegexpErrMemory(ctxt, "compiling regexp");
            xmlFree(stateRemap);
            xmlFree(ret);
    ...
    for (transnr = 0;transnr < nbTrans;transnr++) {
        ...
        if (t1->atom == NULL) {
            ...
            res = xmlFARecurseDeterminism(ctxt, ctxt->states[t1->to],
                                           to, atom);
            state->markd = (xmlRegMarkedType)0;
            if (res == 0) {
                ret = 0;
                /* t1->nd = 1; */
            }
...
xmlFAParseAtom(xmlRegParserCtxtPtr ctxt) {
    int codepoint, len=1;       // To remove warning with GCC 4.x
